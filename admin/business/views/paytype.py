...
class PaytypeBatchDeleteAPIView(APIView):
    def post(self, request):
        ids = request.data
        try:
            Paytype.objects.filter(id__in=ids).delete()
            return SuccessResponse(msg="删除成功")
        except:
            return ErrorResponse(msg="删除失败")


class PaytypeExport(APIView):
    model = Paytype
    ...
    def get(self, request, *args, **kwargs):
        ...
        response['Content-Disposition'] = 'attachment; filename="Paytype.xlsx"'

        # 创建Excel工作簿和工作表
        wb = Workbook()
        sheet = wb.active

        model = apps.get_model('business','Paytype')

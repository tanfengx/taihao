...
class BusinessView(APIView):
    ...
    def post(self, request):
        ...
        user = User.objects.create(username=request.data['username'],name=request.data['username'],uid=uid,password=hashed_password,role='business',email=email,address='')
...
class BusinessBatchDeleteAPIView(APIView):
    def post(self, request):
        ids = request.data
        try:
            Business.objects.filter(id__in=ids).delete()
            return SuccessResponse(msg="删除成功")
        except:
            return ErrorResponse(msg="删除失败")


class BusinessExport(APIView):
    model = Business
    ...
    def get(self, request, *args, **kwargs):
        ...
        response['Content-Disposition'] = 'attachment; filename="Business.xlsx"'

        # 创建Excel工作簿和工作表
        wb = Workbook()
        sheet = wb.active

        model = apps.get_model('business','Business')

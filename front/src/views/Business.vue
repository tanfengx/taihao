...
const confirmDelBatch = () => {
  ...
  request.post('/business/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete('/business/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/business/export`)
}
...
<template>
  <div>
    ...
    <div class="card"  style="text-align: right;">
    <div>
      <el-button type="success" @click="handleAdd" v-if="auths.includes('business.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> 添加 </span>
      </el-button>
      <el-upload
          v-if="auths.includes('business.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/business/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('business.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('business.deleteBatch')">
        <template #reference>
    ...
    <div>
      ...
      <el-table-column prop="username" label="登录账号"></el-table-column>
        ...
        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" size="small" @click="handleEdit(scope.row)" v-if="auths.includes('business.edit')">修改</el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('business.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="商家信息" width="40%">

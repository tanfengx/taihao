...
<template>
  <div>
    ...
    <div>
      ...
      <el-table-column prop="default_radio" label="是否默认"></el-table-column>
      ...
      <el-table-column label="用户编号"><template #default="scope"><span v-if="scope.row.user_id">{{ state.userOptions.find(v => v.id === scope.row.user_id) ? state.userOptions.find(v => v.id === scope.row.user_id).name : '' }}</span></template></el-table-column>
        ...
        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" size="small" @click="handleEdit(scope.row)" v-if="auths.includes('address.edit')">修改</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" title="收货地址信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        ...
        <el-form-item prop="defaultRadio" label="是否默认">
          <el-radio-group v-model="state.form.default_radio">

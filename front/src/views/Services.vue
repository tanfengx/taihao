...
const confirmDelBatch = () => {
  ...
  request.post('/services/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/services/export`)
}
...
<template>
  <div>
    ...
    <div class="card"  style="text-align: right;">
    <div>
      <el-button type="success" @click="handleAdd" v-if="auths.includes('services.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> 添加 </span>
      </el-button>
      <el-upload
          v-if="auths.includes('services.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/services/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('services.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('services.deleteBatch')">
        <template #reference>
    ...
    <div>
      ...
      <el-table-column label="用户"><template #default="scope"><span v-if="scope.row.user_id">{{ state.userOptions.find(v => v.id === scope.row.user_id) ? state.userOptions.find(v => v.id === scope.row.user_id).name : '' }}</span></template></el-table-column>
        ...
        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" size="small" @click="handleEdit(scope.row)" v-if="auths.includes('services.edit')">修改</el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('services.delete')">
              <template #reference>

...
<template>
  <div>
    ...
    <div>
      ...
      <el-table-column label="商品"><template #default="scope"><span v-if="scope.row.goods_id">{{ state.goodsOptions.find(v => v.id === scope.row.goods_id) ? state.goodsOptions.find(v => v.id === scope.row.goods_id).name : '' }}</span></template></el-table-column>
      <el-table-column label="用户"><template #default="scope"><span v-if="scope.row.user_id">{{ state.userOptions.find(v => v.id === scope.row.user_id) ? state.userOptions.find(v => v.id === scope.row.user_id).name : '' }}</span></template></el-table-column>
        ...
        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" size="small" @click="handleEdit(scope.row)" v-if="auths.includes('collect.edit')">修改</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" title="自行车收藏信息" width="40%">

...
<template>
  <div>
    ...
    <div>
      ...
      <el-table-column label="商品分类"><template #default="scope"><span v-if="scope.row.category_id">{{ state.categoryOptions.find(v => v.id === scope.row.category_id) ? state.categoryOptions.find(v => v.id === scope.row.category_id).name : '' }}</span></template></el-table-column>
      <el-table-column label="详情"><template #default="scope"><el-button @click="view(scope.row.content)">查看</el-button></template></el-table-column>
      ...
      <el-table-column prop="remarks" label="审核说明"></el-table-column>
      <el-table-column prop="state_radio" label="状态"></el-table-column>
      <el-table-column label="发布商家"><template #default="scope"><span v-if="scope.row.user_id">{{ state.userOptions.find(v => v.id === scope.row.user_id) ? state.userOptions.find(v => v.id === scope.row.user_id).name : '' }}</span></template></el-table-column>
      <el-table-column prop="views" label="浏览量"></el-table-column>

        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" size="small" @click="handleEdit(scope.row)" v-if="auths.includes('goods.edit')">修改</el-button>
    ...
    />
        ...
        <el-form-item prop="remarks" label="审核说明" v-if="user.role=='ADMIN'">
          <el-input v-model="state.form.remarks" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="stateRadio" label="状态" v-if="user.role=='ADMIN'">
